...
async def update_a_student(request: Request):
    ...
    student_to_upadte = get_a_student_by_id(student_id)

    if request.method == "POST":
        student_update_data = await request.form()

        update_a_student_data(student_id, student_update_data)
        return RedirectResponse(request.url_for("html_endpoint"), status_code=303)
    context = {"request": request, "student": student_to_upadte}
...
routes = [
    ...
    Route("/", html_endpoint),
    Route("/create_student", create_students, methods=["GET", "POST"]),
    Route(
        "/update_student/{student_id:int}/",
        endpoint=update_a_student,
        methods=["GET", "POST"],
    ),
